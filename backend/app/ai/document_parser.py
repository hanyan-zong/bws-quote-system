...
- room_rate (房型多档季节价 ★v0.9.4 新): hotel_name_zh, room_type, season_band (low/shoulder/high/peak/holiday), cost_idr, valid_from, valid_to, note
- season_calendar (季节日历区间 ★v0.9.4 新): name (如 "2026 圣诞旺季"), season_band, date_from, date_to, priority (整数; holiday=10 / peak=8 / high=5 / shoulder=2 / low=1), destination_code (可空), note
- surcharge (酒店附加费 ★v0.9.4 新): hotel_name_zh (空=全局), name, charge_type (tax/service_fee/resort_fee/tourist_tax/summer_break/winter_break/national_day_break/other), calc_method (percent/fixed_per_room_night/fixed_per_pax_night/fixed_per_stay), amount, season_band (可空; 多档逗号 "high,peak"), valid_from, valid_to, note
  · 学校假期类: 中国家长高峰, 通常配 valid_from/to 限定生效期间 — summer_break 配 7-8 月, winter_break 配次年 1-2 月, national_day_break 配 10/1-10/7
- hotel_package (节日/季节捆绑包 ★v0.9.4 新): hotel_name_zh, name, season_band, valid_from, valid_to, mandatory (true/false), cost_idr_per_room, cost_idr_per_pax, includes, replaces_dinner (true/false), note
...
示例 5 — 酒店季节多档价 (★ v0.9.4 新, 优先于示例 1 的二档):
  原文: "巴厘岛 The Mulia | Deluxe Room | 淡季 1.5M | 平季 2.2M | 旺季 3.5M | 圣诞 5.8M"
  说明: 一行多档拆成 4 条 room_rate, 用统一的 hotel_name_zh + room_type 关联.
  输出 (返回为 resources 数组多条):
    {"resource_type":"room_rate","confidence":0.95,"data":{"hotel_name_zh":"巴厘岛 The Mulia","room_type":"Deluxe Room","season_band":"low","cost_idr":1500000}}
    {"resource_type":"room_rate","data":{"hotel_name_zh":"巴厘岛 The Mulia","room_type":"Deluxe Room","season_band":"shoulder","cost_idr":2200000}}
    {"resource_type":"room_rate","data":{"hotel_name_zh":"巴厘岛 The Mulia","room_type":"Deluxe Room","season_band":"high","cost_idr":3500000}}
    {"resource_type":"room_rate","data":{"hotel_name_zh":"巴厘岛 The Mulia","room_type":"Deluxe Room","season_band":"holiday","cost_idr":5800000,"note":"圣诞档"}}

示例 6 — 季节日历 (★ v0.9.4 新):
  原文: "2026/12/20 - 2027/01/05 圣诞新年旺季加价"
  输出: {"resource_type":"season_calendar","data":{"name":"2026 圣诞新年","season_band":"holiday","date_from":"2026-12-20","date_to":"2027-01-05","priority":10}}

示例 7 — 附加费 (★ v0.9.4 新):
  原文: "Government Tax 21% | Service Charge 10% | Tourist Tax IDR 150,000/人/晚"
  输出 (返回为 resources 数组 3 条):
    {"resource_type":"surcharge","data":{"name":"Government Tax 21%","charge_type":"tax","calc_method":"percent","amount":21}}
    {"resource_type":"surcharge","data":{"name":"Service Charge 10%","charge_type":"service_fee","calc_method":"percent","amount":10}}
    {"resource_type":"surcharge","data":{"name":"Tourist Tax","charge_type":"tourist_tax","calc_method":"fixed_per_pax_night","amount":150000}}
  说明: hotel_name_zh 留空 = 全局生效 (适合政府税/旅游税). 若文档明确标某酒店专属 Resort Fee 才填.

示例 7b — 学校假期附加费 (★ v0.9.4):
  原文: "暑期 7/1-8/31 加价 IDR 200,000/人/晚 | 寒假 1/20-2/20 加价 IDR 150,000/人/晚 | 国庆 10/1-10/7 加价 20%"
  输出 (3 条; valid_from/to 限定生效期, 否则会全年误算):
    {"resource_type":"surcharge","data":{"name":"Summer Break Surcharge","charge_type":"summer_break","calc_method":"fixed_per_pax_night","amount":200000,"valid_from":"2026-07-01","valid_to":"2026-08-31"}}
    {"resource_type":"surcharge","data":{"name":"Winter Break Surcharge","charge_type":"winter_break","calc_method":"fixed_per_pax_night","amount":150000,"valid_from":"2027-01-20","valid_to":"2027-02-20"}}
    {"resource_type":"surcharge","data":{"name":"National Day Surcharge","charge_type":"national_day_break","calc_method":"percent","amount":20,"valid_from":"2026-10-01","valid_to":"2026-10-07"}}

示例 8 — 节日捆绑包 (★ v0.9.4 新):
  原文: "圣诞夜 2026/12/24 Gala Dinner 必选: IDR 2,800,000/房 + 500k/人, 含烟花 + 红酒"
  输出: {"resource_type":"hotel_package","data":{"hotel_name_zh":"巴厘岛 The Mulia","name":"Christmas Eve Gala Dinner","season_band":"holiday","valid_from":"2026-12-24","valid_to":"2026-12-24","mandatory":true,"cost_idr_per_room":2800000,"cost_idr_per_pax":500000,"includes":"Gala Dinner + 烟花 + 红酒","replaces_dinner":true}}
  说明: replaces_dinner=true 表示当晚另设晚餐会被自动跳过, 避免双算.
...
7. (v0.9.4) 季节档识别词表:
   - "淡季 / 平日 / 非旺" → low
   - "标准 / 平季 / 常规" → shoulder
   - "旺季 / 高峰 / High Season / Peak" → high
   - "暑假 / 国庆 / 黄金周 / 春节" → peak
   - "圣诞 / 元旦 / 新年 / Xmas / NYE / 跨年" → holiday
8. (v0.9.4) 优先抽取 room_rate(多档) 而非 hotel_room(两档); hotel_room 仅在文档只给一对低/高时用
