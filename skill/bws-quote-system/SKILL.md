...
| **v0.9.3** | **2026-05-19** | **行程类型 day_type + 复制天 + 景点路线 UI 美化** — QuoteDay 加 `day_type: str` (full/half/arrival/departure), pricing_engine 对 half/arrival/departure 车导 × 0.5, departure 自动跳 hotel + breakfast ON. Alembic `0001_quote_day_day_type` migration (idempotent, 检测列存在跳过). 前端每天卡片头加 4 选 1 单选 + 绿色 📋 复制按钮(深拷贝插入下一位). 景点列表 UI 重写: 圆形彩色徽章(出发绿/终点红/中间蓝) + 垂直竖线 + ▶出发/■终点 标签 + 📍 区域 chip. 修长者 input-number width:100% (之前数字框被挤<20px 输不进去) + 目的地下拉同修. 新增 8 unit test (MagicMock 直调 calculate). 67 PASS / 32s |
| CLI | 2026-05-30 | **`bws version` 命令组(根治版本号漂移)** — `version show` 审计 8 处 canonical 版本位置(pyproject / app.__version__ / main.py × 3 / cli desc / index.html × 2)是否一致;`version bump <part\|X.Y.Z>` 改 8 处 → 自动 `pip install -e .` → 清 ~* 残骸 → 校验。**原子**:重装失败自动回滚字符串(不留半完成漂移)。**EOL 字节级保真**(newline="" 读写,不让 LF↔CRLF 被 Windows 文本模式翻译)。离线友好(build isolation 失败退 `--no-build-isolation`,仍失败给联网提示)。13 黑盒/单测 + 2 EOL 回归测试。**106 PASS**(93→106)。注:CLI 工具版,不改业务功能;真 bump 0.9.3→0.9.4 当时因离线(无网+venv 缺 setuptools)未落地,联网后跑 `bws version bump patch` 即可 |
...
bws doctor  [--fix]                     环境自检: Python 版本 / 包安装+entry-point / pyproject↔已装版本同步 / site-packages ~* 残骸 / DB 核心表 / alembic rev↔head; --fix 删 ~* 残骸 (唯一安全自动修复), 其余打印修复命令. 任一 FAIL → 退码 1
bws version show                        审计版本号 8 处 canonical 位置 + 已装版本是否一致 (不一致退码 1)
            bump <major|minor|patch|X.Y.Z>  改 8 处 → 自动 pip install -e . → 清 ~* 残骸 → 校验. 原子: 重装失败自动回滚字符串. --dry-run 预览 / --no-reinstall 只改串 / --no-clean 跳过清残骸
```

> **`bws version bump` = doctor "版本同步" WARN 的根治**。doctor 只检测漂移, bump 一键修。版本号散落 8 处 (pyproject / app.__version__ / main.py × 3〔FastAPI title+health+label〕 / cli description / index.html × 2〔头+脚〕), 历史标记注释 `# v0.9.x:` / migration 头 / few-shot 示例**不动**。底层认知: editable 安装把 version 烤进包元数据, 改 pyproject 不重装则 `metadata.version()` 永远是旧值 — 唯一根治是改完自动重装。离线 (连不上 pypi 且 venv 缺 setuptools) 时重装会失败并自动回滚, 联网后先 `pip install setuptools wheel` 再重跑。
...
│   ├── doctor_cmd.py     ← 2026-05-29 新增, bws doctor 环境自检
│   └── version_cmd.py    ← 2026-05-30 新增, bws version show/bump 版本统一管理+自动重装 (根治漂移, EOL 字节级保真, 原子回滚)
└── backend/tests/        ← 2026-05-16 新增, pytest 用例
    ...
    ├── test_cli_server.py
    ├── test_cli_dev.py
    └── test_cli_doctor.py  ← 2026-05-29 新增, 7 用例 (子进程黑盒 + 残骸/版本 in-process 单测)
```
...
- 位置: `backend/tests/test_cli_*.py`(CLI) + `backend/tests/web/`(Web TestClient),全套 **93 用例**(2026-05-29)
- 跑法: `.venv\Scripts\python.exe -m pytest backend/tests/ -v` (约 33s)
...
```bash
.venv/Scripts/python.exe -m app.cli doctor       # 2026-05-29 起首选: 一条命令查 entry-point/残骸/版本/alembic
.venv/Scripts/python.exe -m pytest backend/tests/ -x  # 必须全绿 (93 用例, 约 33s)
```

`bws doctor` 任一项 FAIL → **不要开工**,先按它打印的修复命令修。
...
### 2. 升级版本号 — 用 `bws version bump`,别手改

```bash
bws version bump patch          # 0.9.3 → 0.9.4, 改 8 处 canonical + 自动 pip install -e . + 清残骸 + 校验
bws version bump 1.0.0          # 或显式版本号
bws version show                # 只读审计: 8 处是否一致 + 已装版本是否同步
```

**2026-05-30 起首选这个**,根治两个老坑:① 版本号散 8 处手改漏改;② 改了 version 没重装导致 `metadata.version()` 漂移。bump 原子(重装失败自动回滚字符串)、EOL 字节级保真。

底层(仍要知道):editable 安装把 version 烤进包元数据,**改 pyproject 不重装则已装版本永远是旧的**。若手改, 必跑:

```bash
.venv/Scripts/python.exe -m pip install -e .   # editable 重装,刷新 entry-point + 元数据
```
...
**2026-05-29 起 `bws doctor` 已实现**:`bws doctor` 会把 `~*` 残骸 + entry-point 缺失 + pyproject↔已装版本漂移 + alembic 落后一并检出;`bws doctor --fix` 自动删 `~*` 残骸(其余打印修复命令,不替用户跑 pip/migrate)。开工 60 秒自检可直接用它替代手动三件套。
